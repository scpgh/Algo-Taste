...
export default function RecipeCard({ recipe, variant = "default" }) {
      ...
      <Link href={data.href}>
        <Card className="rounded-none overflow-hidden border-border hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group pt-0">
          ...
          {data.showImage ? (
            ...
            <div className="relative aspect-square bg-gradient-to-br from-gold-400 via-gold-300 to-gold-200 flex items-center justify-center">
          ...
          {/* Title */}
          <CardHeader>
            <CardTitle className="text-lg font-bold text-foreground group-hover:text-gold-400 transition-colors line-clamp-2">
              {data.title}
            </CardTitle>
          </CardHeader>
        </Card>
      ...
      <Card className="rounded-none border-border hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap gap-2 mb-3">
                {data.cuisine && (
                  <Badge
                    variant="outline"
                    className="text-gold-400 border-gold-400/30 capitalize"
                  >
                    {data.cuisine}
                  </Badge>
                )}
        ...
        <CardContent className="space-y-4 flex-1">
          ...
          {data.missingIngredients && data.missingIngredients.length > 0 && (
            <div className="p-4 bg-gold-50 dark:bg-gold-900/20 border border-gold-400/30">
              ...
              <div className="flex flex-wrap gap-2">
                {data.missingIngredients.map((ingredient, i) => (
                  <Badge
                    key={i}
                    variant="outline"
                    className="text-gold-600 dark:text-gold-400 border-gold-400/30 bg-card"
                  >
                    {ingredient}
                  </Badge>
                ))}
  ...
  if (variant === "list") {
    return (
      <Link href={data.href}>
        <Card className="rounded-none border-border hover:shadow-lg hover:border-gold-400/30 transition-all cursor-pointer group overflow-hidden py-0">
          <div className="flex flex-col md:flex-row">
            ...
            {data.showImage ? (
              ...
              <div className="relative w-full md:w-48 aspect-video md:aspect-square flex-shrink-0 bg-gradient-to-br from-gold-400 to-gold-300 flex items-center justify-center">
            ...
            <div className="flex-1 py-5">
              <CardHeader>
                ...
                <CardTitle className="text-xl font-bold text-foreground group-hover:text-gold-400 transition-colors">
                  {data.title}
                </CardTitle>
